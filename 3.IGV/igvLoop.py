...
import sys
#modify point
igvScript = "TE.M.igv.batch"
sample = sys.argv[1]
bamDir = "/home/songlizhi/sports/swo/align/"
bam    = sample + ".M_TE.bam.realigned.markdup.bam"
disAln = pd.read_csv("/home/songlizhi/sports/swo/align/M_TE_detect/"+sample+".M.disAlign.txt",header=None,names=["id"])
TEAlns = glob.glob("/home/songlizhi/sports/swo/align/M_TE_detect/"+sample+".M-TE*.TEalign.txt")
dfs = []
if not os.path.exists("{sample}.M".format(sample=sample)):
	os.system("mkdir {sample}.M".format(sample=sample))
for TEAln in TEAlns:
...
Odf.to_csv("/home/songlizhi/sports/swo/align/M_TE_detect/{sample}.M.TEins.csv".format(sample=sample))
for index in Adf.index:
	...
	with open(igvScript, "w") as script:
		...
		print("genome /home/songlizhi/genome/swoT2T/T2T_TE/swoM_TE.fa", file = script)
		print("load " + bamDir + bam, file = script)
		print("snapshotDirectory {sample}.M".format(sample=sample), file = script)
	...
	#modify point
	os.system("/home/songlizhi/software/IGV_Linux_2.14.1/igv.sh --batch TE.M.igv.batch")
